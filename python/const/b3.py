...
class _Validator(type,metaclass=_MCValidator):
    def __new__(metacls, cls, bases, clsdict):
        #print(metacls,cls,bases,clsdict)
        a= super().__new__(metacls, cls, bases, clsdict)
        a.__slots__=()
    ...
    def __setattr__(cls, name, value):
        if name=="__slots__" or name=="__dict__":
...
class _CONST(metaclass=_Validator):
    __slots__ = ()  #TODO: how to not have to specify this in this subclass!
    #__dict__ = ""
    #__weakref__ = ()
    VER="0.0.1"
    def __delattr__(self, name):
