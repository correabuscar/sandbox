...
class MyQListWidget(QListWidget):
    ...
    def save_items(self) -> bool:
        with open("/tmp/grub.cfg.mod","w") as f:
            for idx in range(self.count()):
                item=self.item(idx)
    ...
    def __init__(self, parent: QWidget=None):
        ...
        with open("/tmp/grub.cfg", "r") as f:
            ...
            for line in f:
                ...
                if sline.startswith("menuentry "):
                    ...
                    if "" != last_captured_nonme:
                        ...
                        item.setData(MENUENTRY_TITLE, "hidden coalesced non-MENUENTRY grub code from grub.cfg")
            ...
            #TODO: this code block is duplicated! dedupe it!
            if "" != last_captured_nonme:
                #need to write out the so far collected non-ME block:
                item=MyItem()
                item.setData(GRUB_CFG_TEXT, last_captured_nonme)
    ...
    def on_close_attempt(self,event) -> None:
        ...
        if reply == QMessageBox.Close:
            ...
            raise Exception("impossible "+reply)
