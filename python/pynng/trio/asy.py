#!/bin/python3

#src: https://pynng.readthedocs.io/en/latest/core.html#available-protocols
#src: https://github.com/codypiersall/pynng/pull/52/files

from trio import run # type: ignore
from pynng import Pair0, TLSConfig # type: ignore

SERVER_CERT = """
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"""

SERVER_KEY = """
-----BEGIN PRIVATE KEY-----

nFcJUXxIIJLAKbqnMRIp46OP
-----END PRIVATE KEY-----
"""

# we use a self-signed certificate
CA_CERT = SERVER_CERT

import time

async def send_and_recv():
    URL = 'tls+tcp://127.0.0.1:13131'
    TIMEOUTS = 1000 #ms
    # in real code you should also pass recv_timeout and/or send_timeout
    with Pair0(recv_timeout=TIMEOUTS, send_timeout=TIMEOUTS) as server, \
            Pair0(recv_timeout=TIMEOUTS, send_timeout=TIMEOUTS) as client:
        c_server = TLSConfig(TLSConfig.MODE_SERVER,
                                 own_key_string=SERVER_KEY,
                                 own_cert_string=SERVER_CERT,
                                 #auth_mode=TLSConfig.AUTH_MODE_REQUIRED #TODO: make this work? aka server must auth the client!
                                 )
        server.tls_config = c_server
        c_client = TLSConfig(TLSConfig.MODE_CLIENT,
                             ca_string=CA_CERT,
                             auth_mode=TLSConfig.AUTH_MODE_REQUIRED)
        client.tls_config = c_client
        server.listen(URL)
        client.dial(URL, block=True) #must block or else it won't raise when server fails to auth the client!
        print("here")
        #time.sleep(5)
        print("here5")

        await server.asend(b'hello client')
        print(await client.arecv())  # prints b'hello client'
        #time.sleep(5) #C-c works here
        await client.asend(b'hi old buddy server, great to see ya')
        print(await server.arecv())  # prints b'hi old buddy server, great to see ya

run(send_and_recv)

