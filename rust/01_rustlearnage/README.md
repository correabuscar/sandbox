...
Currently almost everything (all functions, files, text) in this repository are made by me Emanuel Czirai and I've dedicated this repository to public domain. Code taken from elsewhere(such as the rustbook(s)) should either be obvious(due to directory names or the code within) or (if I don't forget) will be prefixed by `src: ` and the url of its origin.
