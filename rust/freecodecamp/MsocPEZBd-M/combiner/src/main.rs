//src: https://www.youtube.com/watch?v=OhIuoglzu18
mod args;
use args::Args;
...
enum ImageDataErrors {
    ...
    BufferTooSmall,
    UnableToReadImageFromPath(std::io::Error, String),
...
impl FloatingImage {
    fn new(width: u32, height: u32, name:String) -> Self {
        let buffer_capacity = height*width *4; //3655744;
...
fn main() -> Result<(), ImageDataErrors> {
    let args = Args::new();
    let (image_1, image1_format) =  find_image_from_path(args.image_1)?;
    let (image_2, image2_format) =  find_image_from_path(args.image_2)?;
...
fn standardize_size(image_1: DynamicImage, image_2: DynamicImage) -> (DynamicImage, DynamicImage) {
    ...
    println!("smallest w={}, h={}",width,height);
...
fn find_image_from_path(path: String) -> Result<(DynamicImage, ImageFormat), ImageDataErrors> {
    ...
    match Reader::open(&path) {
        ...
        Err(e) => Err(ImageDataErrors::UnableToReadImageFromPath(e,path))
    }
}
...
fn alternate_pixels(vec_1: Vec<u8>, vec_2: Vec<u8>) -> Vec<u8> {
    let l=vec_1.len();
    let mut combined_data = vec![0u8; l];
    let mut i=0;
    assert!(l % 4 == 0);
    while i<l {
        if i %8 == 0 {
