#[macro_use] //doc: file:///home/user/.rustup/toolchains/nightly-x86_64-unknown-linux-gnu/share/doc/rust/html/book/first-edition/macros.html#scoping-and-macro-importexport
...
// eg. /home/user/build/2nonpkgs/rust.stuff/rustlearnage/compiletime_env
const PROJECT_DIR_AT_COMPILETIME: &'static str = env!("CARGO_MANIFEST_DIR");
const FULLPATH_TO_CARGO_EXE: &'static str = env!("CARGO");
...
//lol my first macro, without reading the docs for how to macro, but used: file:///home/user/.rustup/toolchains/nightly-x86_64-unknown-linux-gnu/share/doc/rust/html/src/std/macros.rs.html#315
...
fn main() {
    ...
    if changed {
        eprintln!("TODO: use $CARGO='{}' for recompilation below? using 'cargo' from PATH now.", FULLPATH_TO_CARGO_EXE);
        ...
        //yes, this is the CARGO used to build the exe the first time! TODO: maybe other vars need
        //to be saved/set here from the time the compilation happened, like idno RUSTC= ?
        let cargo_exe="cargo";
        let output=std::process::Command::new(cargo_exe)
            ...
            //FIXME: Use $CARGO env.var. that was used at build time! eg. maybe capture via build.rs ? so to use the same cargo? otherwise fallback to 'cargo' in PATH ?
            .current_dir(PROJECT_DIR_AT_COMPILETIME)
            .args(&args)
            .output()
            .expect(&format!("failed to execute the '{}' process",cargo_exe));
