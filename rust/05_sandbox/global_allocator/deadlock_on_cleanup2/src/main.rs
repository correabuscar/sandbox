...
unsafe impl GlobalAlloc for MyAllocator {
    //XXX: the logic here with just AtomicBool(s) assumes one thread only; don't use this for multithreads, u'd need the AtomicBools to be threadlocals or something!
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if !ALREADY_BEING_HERE.load(Ordering::Relaxed) {
            ...
            if PANIC_ON_ALLOC.load(Ordering::Relaxed) {
                ...
                match ALREADY_BEING_HERE.compare_exchange(
                    false,
                    true,
                    Ordering::Relaxed,
                    Ordering::Relaxed,
                    ...
                    Ok(prev) => {
                        ...
                        //using std::process::abort() here would be preferred but it creates coredumps which I might want to avoid.
                        ...
                        {
                            let _ = ALREADY_BEING_HERE.compare_exchange(
                                true,
                                false,
                                Ordering::Relaxed,
                                Ordering::Relaxed,
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        if !HAPPENED_ONCE_ALREADY.load(Ordering::Relaxed) {
            ...
            match HAPPENED_ONCE_ALREADY.compare_exchange(
                false,
                true,
                Ordering::Relaxed,
                Ordering::Relaxed,
            ...
            if PANIC_ON_ALLOC.load(Ordering::Relaxed) {
                ...
                match ALREADY_BEING_HERE.compare_exchange(
                    ...
                    Ok(prev) => {
                        assert_eq!(false, prev);
                        panic!(
                            "allocation(zeroed) detected when it shouldn't have allocated anymore!"
                        );
                        //put it back, in case we decide to comment out the panic!() call!
                        #[allow(unreachable_code)]
                        {
    ...
    unsafe fn realloc(&self, ptr: *mut u8, layout: Layout, new_size: usize) -> *mut u8 {
        ...
        if !ALREADY_BEING_HERE.load(Ordering::Relaxed) {
            eprintln!(
                "!! before realloc, oldsize={} newsize={}",
                layout.size(),
                new_size
            );
            match ALREADY_BEING_HERE.compare_exchange(
...
fn main() {
    ...
    //let mut vec = Vec::<i32>::with_capacity(200); //another way to alloc without needing stdout, won't deadlock!
