...
fn main() {
  ...
  println!("Here's OUT_DIR='{}' from within build.rs", env::var("OUT_DIR").expect("$OUT_DIR is not available"));
  for (key, value) in env::vars() {
    println!("{key}: {value}");
  }
  //FIXME: I don't know how to patch cargo for this yet!//println!("Here's CARGO_TARGET_BINFILE_FULLPATH='{}' from within build.rs", env::var("CARGO_TARGET_BINFILE_FULLPATH").expect("You're not using my modified cargo which has a patch applied to make this env.var. available!"));
...
//  println!("cargo:rerun-if-env-changed=A"); //if the environment variable's value changes the build script should be rerun. 
  ...
  println!("cargo:rustc-env=BUILD_DATE={}", 
           ...
           //tm.to_utc().rfc822()
      //time::OffsetDateTime::parse(tm, &Rfc2822)
      //time::OffsetDateTime::parse(tm, &time::format_description::well_known::Rfc2822)?
