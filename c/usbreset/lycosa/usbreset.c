// script found thanks to: <semeion> on freenode irc channel #archlinux
// <semeion> howaboutsynergy: i used it already http://ix.io/1Lh8
...
//dependence: libusb
//local/libusb 1.0.22-1
//    Library that provides generic access to USB devices
//compile: gcc usbreset.c -o usbreset -ggdb -lusb-1.0
//usage: ./usbreset 2 6
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        ...
        printf("Use 'lsusb' to get them.\n");
        printf("Source code is in: /home/user/sandbox/c/usbreset/lycosa\n");
        return 1;
    }
    printf("bus number: %s\n", argv[1]);
    printf("dev number: %s\n", argv[2]);
    ...
    if (l_ret == 0) {
        ...
        printf("error getting usb handle. Probably needs 'root' rights.\n");
        return 1;
    }
