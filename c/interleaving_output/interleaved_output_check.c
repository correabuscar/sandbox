//reported here: https://sourceware.org/bugzilla/show_bug.cgi?id=24621
...
int main(){
    ...
    int size=BUFFERSIZE;//size aka buffersize
    ...
    //XXX WARNING: most comments are outdated!
#elif SETBUFLINE==5
    //from https://stackoverflow.com/review/suggested-edits/23064119
    ...
    for(int i=0; i<400; i++){//XXX this doesn't need to be 2000, unless you're really sure everything is fixed first!! 200 will do until then! if using 20 instead of 200, then only 94 instead of 124 (of 264) tests will fail! using 400 instead of 200 has same result!
